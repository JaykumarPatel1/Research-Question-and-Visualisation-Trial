...
# variable to indicate if toss winner is the match winner
...
# Hypothesis Testing for Win Margin
# Subset data for the two groups
win_margin_yes <- matchesprep$win_margin[matchesprep$toss_and_match_win == "Yes"]
win_margin_no <- matchesprep$win_margin[matchesprep$toss_and_match_win == "No"]

# two-sample t-test
t_test <- t.test(win_margin_yes, win_margin_no, alternative = "two.sided", var.equal = FALSE)
